...
always @(*) begin
    if (inst_ADDU || inst_ADDIU || inst_LW || inst_SW || inst_LB || inst_LBU || inst_LH || inst_LHU || inst_SB || inst_SH)
        ...
        alu_control_out = 13'b0_0_0_10_000_0000; // SLT
    else if (inst_SLTU || inst_SLTIU)
        alu_control_out = 13'b0_0_0_01_000_0000; // SLTU
    else if (inst_AND || inst_ANDI)
        alu_control_out = 13'b0_0_0_00_100_0000; // AND
    else if (inst_NOR)
        alu_control_out = 13'b0_0_0_00_010_0000; // NOR
    else if (inst_OR || inst_ORI)
        alu_control_out = 13'b0_0_0_00_001_0000; // OR
    else if (inst_XOR || inst_XORI)
        alu_control_out = 13'b0_0_0_00_000_1000; // XOR
        ...
        alu_control_out = 13'b0_0_0_00_000_0000_1; // LUI
    else if (inst_DIV)
        alu_control_out = 13'b1_0_0_00_000_0000; // DIV
    else
        alu_control_out = 13'b0; // 默认 (NOP, J, JR, JAL, JALR, ERET)
end
...
always @(*) begin
    ...
    if (inst_SLL || inst_SRL || inst_SRA)
        ...
        alu_operand1_out = rs_value; // 默认使用 rs
    ...
    if (op == 6'b000000) // R型 (移位除外)
        ...
        alu_operand2_out = rt_value; // 默认 (例如，使用 shamt 的移位)
end
...
always @(*) begin
    ...
    if (inst_JAL)
        rf_wdest_out = 5'd31; // $ra
    else if (inst_JALR)
        rf_wdest_out = rd; // 目标由 rd 指定
